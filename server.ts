...
`;
    ...
    res.status(500).json({
      reply:
        `Thank you for contacting Shyam Machine House. For instant support, please reach our sales team directly on WhatsApp: ${COMPANY_INFO.phone}`,
      whatsappLink:
        `https://wa.me/${COMPANY_INFO.whatsappNumber}?text=${encodeURIComponent(
          'Hello Shyam Machine House, I would like to inquire about your machinery catalog.'
        )}`
    });
  }
});
...
app.post('/api/lead', (req, res) => {
  ...
  res.json({
    ...
    supplierWhatsappUrl,
    message:
      'Your inquiry has been logged successfully! Connecting you to supplier on WhatsApp...'
  });
});
...
async function startServer() {
  ...
  app.listen(
    ...
    () => {
      console.log(
        `Shyam Machine House server running on http://0.0.0.0:${PORT}`
      );
    }
  );
}
